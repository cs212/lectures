...
/**
 * This class illustrates basic mutability using an array and primitive types.
 * Most data structures are mutable, meaning you can modify the content stored
 * by the data structure by passing around the reference.
 *
 * <p><em>
...
public class MutabilityDemo {
	...
	public static void main(String[] args) {
		 ...
		 * keyword. Instead, we are simply creating another way to refer to
		 * the memory already allocated to "a". Think of it as a nickname.
		 *
		...
		/*
		 * References to this memory could be in another scope. For example,
		 * the incrementFirst() method here has a reference to the array's
		 * memory, so changes within that method will be seen outside of the
		 * method as well.
		 */
		...
		/*
		 * Primitive types are different. Mutability is something we really
		 * only discuss in to the context of objects, but you can think of
		 * a primitive type as being immutable. If you modify the value, you
		 * break the original memory reference.
		 */
		 ...
		 * value. When you pass the value to a method, any modifications to
		 * that value are lost when the method scope is exited unless you return
		 * those changes.
		 */
