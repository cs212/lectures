...
public class AdventureServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res)
		...
		template = template.replaceAll("#message#", room.toString());
		template = template.replaceAll("#west#", west);
		template = template.replaceAll("#east#", east);
		template = template.replaceAll("#north#", north);
		template = template.replaceAll("#south#", south);

		// allows session tracking even if cookies are disabled
		template = template.replaceAll("#action#", res.encodeURL("/"));

		// add debug information
		template = template.replaceAll("#session#", session.getId());
		template = template.replaceAll("#room#", String.valueOf(room.ordinal()));
		template = template.replaceAll("#direction#", direction.toString());
